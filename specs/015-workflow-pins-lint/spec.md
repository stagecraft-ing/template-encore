...
# 015 — Workflow-ref SHA-pinning lint
...
contract. It runs as a gate in `ci-supply-chain.yml` (spec 014) at the
...
`ci-supply-chain.yml` (spec 014) via section-anchored co-authority.
...
**AC-5**
```bash
npx spec-spine registry show 015-workflow-pins-lint
```

...
summary: >
  ...
  satisfies the SHA-pin (spec 015) and allow-listed-publisher (spec 016)
...
workflow-owning specs (011–017).

This spec **establishes** `deploy-docs.yml`. Spec 016
(`016-enterprise-actions-governance`) separately **constrains** it (and
every workflow) to allow-listed publishers; spec 015
(`015-workflow-pins-lint`) requires its external refs to be SHA-pinned.
Ownership (this spec) and the cross-cutting constraints (015, 016) compose:
...
40-hex commit SHA (spec 015) and MUST resolve to an allow-listed publisher
(spec 016). The workflow uses only first-party `actions/*` publishers
...
branch-protection and merge-queue carve-outs in specs 013 and 016.
...
**AC-3**
```bash
npx spec-spine registry show 021-docs-website
```
...
- **The in-repo `docs/` tree**: `docs/` is separate prose documentation in
  ...
  not `docs/`.
- **Adding publishers to the org allow-list**: governed by spec 016's

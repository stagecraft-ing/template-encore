...
# 011 — Encore CI/CD: composite actions, CI workflow, CD example, typed-client staleness gate
...
(spec 013) and its result is aggregated by the terminal `ci-gate` job.
...
with spec 013: the `encore` route in `jobs.changes` and the `encore` entry in
...
SHA-pinning every `uses:` (spec 015).
...
full 40-hex commit SHA (spec 015). Path-local refs
...
(SHA-pin check via spec 015 gate in the supply-chain workflow).

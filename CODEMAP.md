...
Encore application (apps/api) ════════════════════════════════════════════
  Gateway + authHandler (auth/handler.ts)   verifies access-token cookie | Bearer → AuthData{ roles, ... }
    ...
    │             authHandler + Gateway; drivers {mock, entra-id, saml}; me/refresh/logout/csrf-token
    │
...
```
Driver selection: AUTH_DRIVER env (mock | entra-id | saml) is the default for /api/v1/auth/login.

  ├── mock      ───► GET /api/v1/auth/mock/login?user=0|1|2 ───► instant principal
  ├── entra-id  ───► GET .../entra-id/login → 302 Microsoft → GET .../entra-id/callback (code exchange)
  └── saml      ───► GET .../saml/login → 302 IdP SSO → POST .../saml/callback (assertion)
                                                              │
                                              ┌───────────────┘
                                              ▼
...
**Driver files**: `apps/api/auth/{mock,entra-id,saml}.ts` · **Handler/Gateway**: `apps/api/auth/handler.ts`
...
auth/        handler.ts (authHandler + Gateway), encore.service.ts (securityHeaders + csrfMiddleware + apiRateLimit),
             drivers.ts (discovery + default login + status), mock.ts, entra-id.ts, saml.ts,
...
2. **Multi-driver auth**: `mock`/`entra-id`/`saml` coexist; `AUTH_DRIVER` sets the default. Uniform discovery/login surface.

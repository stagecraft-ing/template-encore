...
any workflow file — those are owned by their respective specs (011, 012,
013, 014, 017). It constrains all of them via the `constrains:` ownership
edge.

The SHA-pin half of the Actions posture is governed by spec 015
(`015-workflow-pins-lint`). This spec adds the publisher-scope constraint
...
per spec 015. This spec adds the publisher-scope constraint on top:
...
workflow file, consistent with the branch-protection carve-out in spec 013.
...
Spec 015's `workflow-pins.sh` enforces the SHA-pin half (FR-03) at the
...
**AC-4**
```bash
npx spec-spine registry show 016-enterprise-actions-governance
```
exits 0.

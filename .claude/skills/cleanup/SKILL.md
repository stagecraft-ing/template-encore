...
Spawn a cleanup-analyzer sub-agent that runs dead-code and duplicate-code detection across this repo's npm/TypeScript surface (`apps/web`, `apps/web-internal`, `apps/api`, `packages/`, `modules/`, `scripts/`), investigates each finding in context, and returns a structured report with categorized recommendations.
...
#### A.1: TypeScript/JavaScript surface (`apps/*`, `packages/*`, `modules/*`)
...
for f in $(find apps/*/src packages/*/src modules/*/files -type f \
...
if npx --yes --no-install jscpd --version >/dev/null 2>&1; then
  npx --yes --no-install jscpd apps packages modules scripts \
    --min-lines 10 \
    --min-tokens 50 \
...
- **`apps/api/src/modules.ts`**: assembled from installed module manifests (CODEMAP invariant: do not edit by hand); regenerated by the template's module scripts.
...
- **Module manifest files** under `modules/*/manifest.json`: read by the orchestrator at install time.
...
- Module scaffolding under `modules/`: each module is a standalone unit; structural similarity is intentional.

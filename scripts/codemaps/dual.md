# Codemap: Enterprise Dual-App (Public + Internal)
...
```
my-dual-app/
├── public/                        External-facing Encore app (AUTH_DRIVER=saml)
│   ├── apps/
...
│
└── internal/                      Staff-facing Encore app (AUTH_DRIVER=entra-id)
    ├── apps/
    │   ├── api/                   Encore.ts application (standalone)
    │   │   ├── encore.app
    │   │   ├── infra.config.json
...
### Public App (external user-facing, SAML)
...
Encore application (public/apps/api) ════════════════════════════════════
  Gateway + authHandler   verifies access-token cookie | Bearer → AuthData{ roles }
    ...
    │             authHandler + Gateway; driver = saml; me/refresh/logout/csrf-token/saml/*
    │
...
### Internal App (staff-facing, Entra ID)
...
Encore application (internal/apps/api) ══════════════════════════════════
  Gateway + authHandler   verifies access-token cookie | Bearer → AuthData{ roles }
    ...
    │
    ├── auth      securityHeaders + csrfMiddleware + apiRateLimit
    │             authHandler + Gateway; driver = entra-id; me/refresh/logout/csrf-token/entra-id/*
    │
    ├── gateway   api.raw catch-all /api/v1/data/* (auth:true) → private backend (S2S OAuth)
    │
...
internal on 4001). Each has its own `infra.config.json`, JWT keys, and IdP registration. There is no
...
Both apps expose the same core endpoint set, differing only in auth driver and optional modules.
...
### Public-app-only endpoints (SAML)

| Method | Path | Auth | Notes |
|--------|------|:----:|-------|
| GET | `/api/v1/auth/saml/login` | - | SAML redirect to IdP |
| POST | `/api/v1/auth/saml/callback` | - | SAML assertion POST |
| GET | `/api/v1/auth/saml/metadata` | - | SP metadata XML |

### Internal-app-only endpoints (Entra ID; user-management module optional)
...
Internal app (AUTH_DRIVER=entra-id):
...
auth/        handler.ts (authHandler + Gateway), encore.service.ts,
             drivers.ts, {saml|entra-id}.ts, mock.ts, me.ts, refresh.ts, logout.ts,
...
└── AppLayout.vue                       └── AppLayout.vue
    ...
    │   ├── HomeView                        │   ├── HomeView
    │   ├── LoginView (SAML / mock)         │   ├── LoginView (Entra ID / mock)
...
| Driver | Role source | Fallback |
|--------|-------------|----------|
| **Entra ID** | Token claims: `roles` → `role` → `groups` | `ENTRA_DEFAULT_ROLE` (default: `user`) |
| **Mock** | Hardcoded in `apps/api/auth/mock.ts` | n/a |

If the **user-management module** is installed in the internal app, app-managed roles replace IdP
claims. Admins manage roles via `/api/v1/admin/*`.
...
2. **Trust-zone separation**: different IdPs (SAML vs Entra ID), different JWT key pairs, different `infra.config.json`, different network exposure boundaries.
...
| `docs/AUTH-SETUP.md` | SAML + Entra ID driver configuration |
